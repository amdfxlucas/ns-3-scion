...
std::expected<input_t,error> decodeSCMPExternalInterfaceDown(input_t , LayerStore* );
...
class SCMPExternalInterfaceDown : public virtual Layer, public Header
{
    public:
    SCMPExternalInterfaceDown(){}
    SCMPExternalInterfaceDown( IA_t ia, ASIFID_t id):m_ia(ia), m_ifid(id) {}
...
private:
    ...
    inline static LayerType m_type = LayerTypeRegistry::instance().RegisterLayerType(
                                LayerTypeMetaData{
                                .m_name = "SCMPExternalInterfaceDown",
                                .m_decoder = &decodeSCMPExternalInterfaceDown     });
