                        ...
                        const YAML::Node& config)
{
    ...
    while (cur_xml_node)
    {
        ...
        if (type == "core")
        {
            as_node = CreateObject<ScionCoreAs>(0,
                                                (alias_as_no == 0),
                                                alias_as_no,
                                                cur_xml_node,
                                                config,
        ...
        {
            as_node = CreateObject<ScionAs>(0,
                                            (alias_as_no == 0),
                                            alias_as_no,
                                            cur_xml_node,
                                            config,
...
void
GetMaliciousTimeRefAndTimeServer(const NodeContainer& as_nodes,
                                 const YAML::Node& config,
...
void
GetTimeServiceSnapShotTypes(const NodeContainer& as_nodes,
                            const YAML::Node& config,
...
void
GetTimeServiceAlgVersions(const NodeContainer& as_nodes,
                          const YAML::Node& config,
