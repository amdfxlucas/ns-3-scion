...
class TimeServer : public ScionHost
{
  public:
    TimeServer(uint32_t system_id,
               ...
               std::string read_disjoint_paths,
               std::string time_service_output_path,
               std::string reference_time_type,
  ...
  private:
    ...
    std::map<std::string, TimeServerType> time_server_type_map = {
        {"NORMAL", TimeServerType::NORMAL},
        {"MALICIOUS", TimeServerType::MALICIOUS_SERVER}};
