                           ...
                           const YAML::Node& config)
{
    ...
    as_max_bwd = 0;
    for (const auto curr_bwd : inter_as_bwds)
    {
                           ...
                           bool only_propagation_delay)
{
    ...
    for (const auto& br : border_routers)
    {
        br->InitializeTransmissionQueues();
    }

    for (int test = 0; const auto& host : hosts)
    {
        ++test;
...
ScionAs::ConnectInternalNodes(bool only_propagation_delay)
{
        ...
        !only_propagation_delay)
    {
        ...
        std::uniform_int_distribution<uint64_t> dist(
            50000,
            300000); // random asymmetry between 50ms and 300ms
    ...
    for (uint32_t i = 0; i < border_routers_vec.size(); ++i)
    {
        ...
        for (uint32_t j = 0; j < hosts.size(); ++j)
        {
            ...
            Time propagation_delay = NanoSeconds(
                (int64_t)floor(1e6 * CalculateGreatCircleLatency((ld)br->GetLatitude(),
                                                                 (ld)br->GetLogitude(),
                                                                 (ld)host->GetLatitude(),
                                                                 (ld)host->GetLogitude())));
    ...
    for (uint32_t i = 0; i < border_routers_vec.size(); ++i)
    {
        ...
        br->AddToRemoteNodesInfo(path_server,
    ...
    for (uint32_t i = 0; i < hosts.size(); ++i)
    {
        ...
        Time propagation_delay = NanoSeconds(
            (int64_t)floor(1e6 * CalculateGreatCircleLatency((ld)host->GetLatitude(),
                                                             (ld)host->GetLogitude(),
                                                             (ld)path_server->GetLatitude(),
                                                             (ld)path_server->GetLogitude())));
        ...
        host->AddToRemoteNodesInfo(path_server,
                                   path_server->GetNDevices() - 1,
                                   isd_number,
                                   as_number);
    ...
    for (uint32_t i = 0; i < hosts.size() - 1; ++i)
    {
        ...
        for (uint32_t j = i + 1; j < hosts.size(); ++j)
        {
            ...
            Time propagation_delay = NanoSeconds(
                (int64_t)floor(1e6 * CalculateGreatCircleLatency((ld)h1->GetLatitude(),
                                                                 (ld)h1->GetLogitude(),
                                                                 (ld)h2->GetLatitude(),
                                                                 (ld)h2->GetLogitude())));
...
void
ScionAs::InstantiateBeaconServer(bool parallel_scheduler,
                                 rapidxml::xml_node<>* xml_node,
                                 const YAML::Node& config)
{
