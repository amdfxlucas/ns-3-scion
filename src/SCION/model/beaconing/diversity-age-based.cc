...
void
DiversityAgeBased::DoInitializations(uint32_t num_ases,
...
DiversityAgeBased::AlgSpecificImportPolicy(Beacon& the_beacon,
                                           uint16_t sender_as,
...
void
DiversityAgeBased::InsertToAlgorithmDataStructures(Beacon* the_beacon,
                                                   uint16_t sender_as,
                                                   uint16_t remote_egress_if_no,
                                                   uint16_t self_ingress_if_no)
{
            ...
            for (const auto& self_egress_if_no : interfaces)
            {
                ...
                else
                {
                    ...
                    score = std::pow(raw_score,
                                     std::pow(BETA * (sent_beacon_time_to_expiration /
    ...
    for (uint32_t i = 0; i < MAX_BEACONS_TO_SEND; ++i)
    {
        ...
        else
        {
            ...
            if (PathNotSentBefore(remote_as_no, max_score_iface, max_score_beacon))
            {
                ...
                AddToSentBeacons(dst_as_no,
                                 remote_as_no,
                                 max_score_iface,
                                 max_score_beacon,
                                 (float)max_score_raw_score);
                IncLinksJointnessOnSentPaths(dst_as_no,
                                             remote_as_no,
                                             max_score_iface,
                                             max_score_beacon);
            }
...
inline ld
DiversityAgeBased::CalculateRawScore(Beacon* the_beacon,
                                     uint16_t dst_as_no,
                                     uint16_t self_egress_if_no,
                                     ScionAs* remote_as)
{
...
void
DiversityAgeBased::UpdateSentBeaconTimer(uint16_t remote_as,
                                         uint16_t self_egress_if_no,
                                         Beacon* the_beacon)
{
...
void
DiversityAgeBased::IncLinksJointnessOnSentPaths(uint16_t dst_as_no,
                                                uint16_t remote_as_no,
                                                uint16_t self_egress_if_no,
                                                Beacon* the_beacon)
{
    if (the_beacon != NULL)
    {
        const auto& the_path = the_beacon->the_path;
        for (const auto& seg : the_path)
        {
...
void
DiversityAgeBased::AddToSentBeacons(uint16_t dst_as_no,
                                    uint16_t remote_as,
                                    uint16_t self_egress_if_no,
                                    Beacon* the_beacon,
                                    float raw_score)
...
bool
DiversityAgeBased::PathNotSentBefore(uint16_t remote_as,
                                     uint16_t self_egress_if_no,
                                     Beacon* the_beacon)
{
...
void
DiversityAgeBased::DecLinksJointnessesOnSentPaths(Beacon* the_beacon,
                                                  uint16_t dst_as,
                                                  uint16_t remote_as_no,
