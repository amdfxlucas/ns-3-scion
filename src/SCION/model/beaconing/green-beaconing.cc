...
void
GreenBeaconing::DoInitializations(uint32_t num_ases,
                                  ...
                                  const YAML::Node& config)
{
    ...
    for (uint32_t i = 0; i < num_ases; ++i)
    {
        ...
        beacons_per_dst_per_ing_if_sorted_by_pollution.at(i).resize(GetAs()->GetNInterfaces());
...
GreenBeaconing::AlgSpecificImportPolicy(Beacon& the_beacon,
                                        uint16_t sender_as,
...
void
GreenBeaconing::InsertToAlgorithmDataStructures(Beacon* the_beacon,
                                                uint16_t sender_as,
                                                uint16_t remote_egress_if_no,
                                                uint16_t self_ingress_if_no)
{
    ...
    { // Per neighbor AS
        ...
        { // Per destination AS
            ...
            std::multimap<
                ...
                selected_beacons;
            SelectBeaconsToDisseminatePerDstPerNbr(remote_as_no,
                                                   dst_as_no,
        ...
        pollution_index_map_to_beacon_and_metadata)
{
    ...
    for (const auto& iface_to_pollution_beacon_pair : valid_candidates)
    {
        uint16_t self_egress_if_no = iface_to_pollution_beacon_pair.first;
        int no_beacons_per_iface = 0;
        for (const auto& pollution_beacon_pair : iface_to_pollution_beacon_pair.second)
        {
