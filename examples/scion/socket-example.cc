...
main(int argc, char* argv[])
{
    ...
    Ptr<Socket> srcSocket =
        Socket::CreateSocket(nSrc,  TypeId::LookupByName( "ns3::Ipv4RawSocketFactory")
    ...
    Ptr<Socket> dstSocket =
        Socket::CreateSocket(nDst,   TypeId::LookupByName( "ns3::Ipv4RawSocketFactory")
        // TypeId::LookupByName("ns3::UdpSocketFactory")
        );
...
{
    SCIONHeader scion;
    scion.SetDstAddress( std::string("19-ffaa:0:1067,127.0.0.1:8080") ) ;
    scion.SetSrcAddress( std::string("19-ffaa:0:1067,127.0.0.2:8080") ) ;
...
srcSocketRecv(Ptr<Socket> socket)
{
    ...
    packet->RemoveHeader(scion);
    NS_LOG_INFO( "srcSocketRecv scion packet from: " << scion.SrcAddress() );
...
dstSocketRecv(Ptr<Socket> socket)
{
    ...
    Ptr<Packet> packet = socket->RecvFrom(from);
    packet->RemoveHeader(scion);
      NS_LOG_INFO( "dstSocketRecv scion packet from: " << scion.SrcAddress() );
